...
def test_plot_interaction_runs_without_error(interaction_data):
    ...
    """
    try:
        # Arrange: Get the necessary data and p-value first
        res, clean_df, p_val = calculate_interaction(interaction_data, 'Score', 'Gender', 'Treatment')
        
        # Act: Attempt to plot using the results from the calculation
        # We pass clean_df and the p_val extracted earlier
        plot_interaction_bar(clean_df, 'Gender', 'Treatment', 'Score', p_val)
        
        # Cleanup: Close the plot to prevent windows from popping up during tests
        plt.close('all')
